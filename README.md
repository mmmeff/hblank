...
Stop `hblank dev`, then run from the package containing `.hblank/`:
...
`update` installs the latest stable, fully published crates.io release of the CLI, pins your host and preview Hblank dependencies to that release, and refreshes both lockfiles. It handles workspace-inherited dependencies and preserves comments, features, and existing setup. Updating the CLI alone does not update the UI.

Use `hblank update --project crates/ui` for another package, or `hblank update --version 0.6.0` to choose an exact published release. Commit the changed manifests and tracked lockfiles; do not rerun `hblank init`.

If your CLI does not recognize `update`, install a release containing it first with `cargo install hblank-cli --locked`. Git/path dependencies and alternate registry or Hblank source overrides require a manual update; this command will not convert them or migrate your GPUI backend. A Git push is not a crates.io release. See the [update reference](docs/cli.md#hblank-update) for behavior and failure recovery.
