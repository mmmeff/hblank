...
Hblank is the Rust/GPUI analog to Storybook: glob-discovered component examples, generated property controls, Rustdoc beside the preview, and automatic rebuilds in a dedicated GPUI desktop harness.
...
`--fixture` opens directly to the first registered example in that source file. Relative paths resolve from `--project`; absolute paths work too. If a file contains multiple examples, Hblank chooses the first in deterministic navigation order.

While it runs:

- adding or removing a matching example refreshes navigation;
- changing component or example Rust code triggers a debounced rebuild;
...
Every one has an independently discoverable example in `fixtures/dogfood/src/harness.hblank.rs`:
